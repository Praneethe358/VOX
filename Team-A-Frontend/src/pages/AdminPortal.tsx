...
const AdminSidebar: React.FC<{  activePage: string;  onNavigate: (page: string) => void;  navItems: NavItem[];  adminName?: string;  adminRole?: string;}> = ({ activePage, onNavigate, navItems, adminName, adminRole }) => (  <motion.nav    initial={{ x: -240 }}    animate={{ x: 0 }}    transition={{ duration: 0.3 }}    style={{ width: '240px', minHeight: '100vh', background: 'var(--surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', position: 'fixed', left: 0, top: 0, zIndex: 100 }}  >    <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)' }}>      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>        <svg width="34" height="26" viewBox="0 0 48 36" fill="none"><path d="M 0 24 L 6 24 C 9 24, 9 10, 12 10 C 15 10, 15 24, 18 24 C 21 24, 21 4, 24 4 C 27 4, 27 32, 30 32 C 33 32, 33 16, 36 16 C 39 16, 39 24, 42 24 L 48 24" stroke="var(--wave)" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" fill="none"/></svg>        <span style={{ fontFamily: "'Manrope', sans-serif", fontSize: '20px', fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.5px' }}>VOX</span>      </div>      <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px', letterSpacing: '0.5px' }}>Admin Console</p>    </div>    <div style={{ flex: 1, padding: '12px 10px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '2px' }}>      <div style={{ padding: '8px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1.5px' }}>        Navigation      </div>      {navItems.map((item) => (        <motion.button          key={item.id}          whileHover={{ x: 3 }}          whileTap={{ scale: 0.98 }}          onClick={() => onNavigate(item.id)}          style={{ width: '100%', padding: '10px 16px', borderRadius: '10px', fontSize: '13px', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', border: activePage === item.id ? '1px solid rgba(45,78,232,0.3)' : '1px solid transparent', background: activePage === item.id ? 'rgba(45,78,232,0.1)' : 'transparent', color: activePage === item.id ? 'var(--text)' : 'var(--text-sec)', transition: 'all 0.15s' }}        >          <span style={{ fontSize: '16px' }}>{item.icon}</span>          {item.label}        </motion.button>      ))}    </div>    <div style={{ padding: '16px 20px', borderTop: '1px solid var(--border)' }}>      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>        <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'linear-gradient(135deg, var(--accent), var(--accent-lt))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '13px' }}>          A        </div>        <div>          <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)' }}>{adminName || 'Admin User'}</p>          <p style={{ fontSize: '11px', color: 'var(--green-lt)' }}>● {adminRole || 'Online'}</p>        </div>      </div>    </div>  </motion.nav>);
// ─── Top Bar Component ──────────────────────────────────────────────────────
const AdminTopbar: React.FC<{  pageTitle: string;  onLogout: () => void;}> = ({ pageTitle, onLogout }) => (  <motion.div    initial={{ y: -64 }}    animate={{ y: 0 }}    transition={{ duration: 0.3, delay: 0.1 }}    style={{ height: '56px', background: 'var(--surface)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', position: 'sticky', top: 0, zIndex: 50, backdropFilter: 'blur(12px)' }}  >    <h2 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text)', fontFamily: "'Manrope', sans-serif" }}>{pageTitle}</h2>    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>      <motion.button        whileHover={{ scale: 1.05 }}        whileTap={{ scale: 0.95 }}        onClick={onLogout}        style={{ padding: '6px 14px', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.3)', color: '#f87171', fontWeight: 500, fontSize: '13px', background: 'rgba(239,68,68,0.06)', cursor: 'pointer', transition: 'all 0.15s' }}      >        Logout      </motion.button>    </div>  </motion.div>);
// ─── Stat Card Component ────────────────────────────────────────────────────
const StatCard: React.FC<StatCardData & { delay: number }> = ({ icon, label, value, trend, delay }) => (  <motion.div    initial={{ opacity: 0, y: 20 }}    animate={{ opacity: 1, y: 0 }}    transition={{ duration: 0.3, delay }}    whileHover={{ y: -4 }}    style={{ background: 'var(--surface)', border: '1px solid var(--border2)', borderRadius: '14px', padding: '20px', transition: 'all 0.18s' }}  >    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>      <div>        <p style={{ color: 'var(--text-sec)', fontSize: '13px', fontWeight: 500 }}>{label}</p>        <p style={{ fontSize: '28px', fontWeight: 800, color: 'var(--text)', marginTop: '8px', fontFamily: "'Manrope', sans-serif" }}>{value}</p>        {trend && <p style={{ fontSize: '11px', color: 'var(--green-lt)', marginTop: '6px' }}>{trend}</p>}      </div>      <div style={{ fontSize: '28px' }}>{icon}</div>    </div>  </motion.div>);
// ═══════════════════════════════════════════════════════════════════════════════
//  DASHBOARD SECTION
// ═══════════════════════════════════════════════════════════════════════════════
